...
import { Check, Clock, Edit, Trash2, TrendingUp, Zap } from "lucide-react";
...
export const ResumeSummaryCard = ({
  ...
  //get color according to completion %
  ...
  return (
    <div
      ...
      onClick={onSelect}
      onMouseEnter={() => setIsHovered(true)}//mouse or swipe
...
export const TemplateCard=({thumbnailImg,isSelected,onSelect})=>{
  return(
    <div className={`group h-auto md:h-[300px] lg:h-[320px] flex flex-col bg-white border-2 overflow-hidden cursor-pointer transition-all duration-500 hover:scale-105 hover:shadow-lg rounded-3xl
      ${
        isSelected?'border-yellow-500 shadow-lg shadow-yellow-500/20 bg-violet-50':'border-gray-200 hover:border-yellow-300'
      }`} onClick={onSelect}>
        {thumbnailImg?(
          <div className="relative w-full h-full overflow-hidden">
            ...
            {isSelected && (
              <div className="absolute insert-0 bg-yellow-50/10 flex items-center justify-center">
            ...
            {/*hover effect*/}
            <div className="absolute insert-0 bg-gradient-to-t from-yellow-100/30 to tansparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            </div>
          </div>
        ):(
          <div className="w-full h-[200px] flex items-center flex-col justify-center bg-gradient-to-br from-yellow-50 via-yellow-600 to-fuchsia-50">
            <div className="w-12 h-12 bg-gradient-to-br from-yellow-500 to-fuchsia-500 rounded-2xl flex items-center justify-center mb-3">

...
const EditResume = () => {
  ...
  const validateAndNext = (e) => {
    ...
    switch (currentPage) {
      case "profile-info":
        const { fullName, designation, summary } = resumeData.profileInfo //resumeData.profileInfo
  ...
  //fetching resume deatils using backend url
  ...
  //it will help in choosing the preview as well as downloading the resume
      ...
      <div className={containerStyles.main}>
        ...
        <div className='flex flex-wrap items-center gap-2'>
          ...
          <button onClick={()=>setOpenPreviewModal(true)} className={buttonStyles.download}>
            <Download size={16}/>
            <span className="text-xm">Preview</span>
          </button>
        </div>
        </div>
        {/*step progress*/}
      ...
      {/*Thumbnail error fix*/}
          <div style={{display:"none"}} ref={thumbnailRef}>
            <div className={containerStyles.hiddenThumbnail}>
              <RenderResume 
              key={`thumb-${resumeData?.template?.theme}`} templateId={resumeData?.template?.theme || ""}
              resumeData={resumeData}
              />
            </div>
          </div>
